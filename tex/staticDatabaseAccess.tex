...
\begin{itemize}
\item \index{DCT\_STRING}DCT\_STRING: Character string.

\item \index{DCT\_INTEGER}DCT\_INTEGER: Integer value

\item \index{DCT\_REAL}DCT\_REAL: Floating point number

\item \index{DCT\_MENU}DCT\_MENU: A set of choice strings

\item \index{DCT\_MENUFORM}DCT\_MENUFORM: A set of choice strings with associated form.

\item \index{DCT\_INLINK}DCT\_INLINK: Input Link

\item \index{DCT\_OUTLINK}DCT\_OUTLINK: Output Link

\item \index{DCT\_FWDLINK}DCT\_FWDLINK: Forward Link

\item \index{DCT\_NOACCESS}DCT\_NOACCESS: A private field for use by record access routines
...
\index{dbReadDatabase}\verb|dbReadDatabase| and \index{dbReadDatabaseFP}\verb|dbReadDatabaseFP| both read a file containing database definitions as described in chapter 
...
\index{dbGetNRecordTypes}This routine returns the number of record types in the database.
...
\index{dbGetRecordTypeName}This routine returns the name of the record type that DBENTRY currently references. This routine should only be called 
...
\index{dbGetNFields}Returns the number of fields for the record instance that DBENTRY currently references.
...
\index{dbGetFieldType}This routine returns the integer value for a DCT field type, see Section14.2.3 on page198, for a description of the field 
types.
...
\index{dbGetFieldName}This routine returns the name of the field that DBENTRY currently references. It returns NULL if DBENTRY does not 
point to a field.
...
\index{dbGetDefaultName}This routine returns the default value for the field that DBENTRY currently references. It returns NULL if DBENTRY 
...
\index{dbPutRecordAttribute}This creates or modifies the attribute \emph{name} on the record type referenced by \emph{pdbentry} to \emph{value}. Attribute names should be 
...
\index{dbGetRecordAttribute}Looks up the attribute \emph{name} for the record type referenced by \emph{pdbentry} and sets the the field pointer in \emph{pdbentry} to refer to 
...
\index{dbGetNRecords}Returns the total number of record instances and aliases for the record type that DBENTRY currently references.
...
\index{dbGetNAliases}Returns the number of record aliases for the record type that DBENTRY currently references.
...
\index{dbGetRecordName}This routine only works properly if called after \verb|dbFindRecord|, \verb|dbFirstRecord|, or \verb|dbNextRecord| has returned 
...
\index{dbIsAlias}This routine only works properly if called after \verb|dbFindRecord|, \verb|dbFirstRecord|, or \verb|dbNextRecord| has returned 
...
\index{dbCopyRecord}This routine copies the record instance currently referenced by \verb|DBENTRY| (it fails if DBENTRY references an alias). Thus 
...
\index{dbRenameRecord}This routine renames the record instance currently referenced by \verb|DBENTRY| (it fails if DBENTRY references an alias). If 
...
\index{dbGetNMenuChoices}This routine returns the number of menu choices for menu.
...
\index{dbGetMenuChoices}This routine returns the address of an array of pointers to strings which contain the menu choices.
...
\index{dbFindMenu}\verb|dbFindMenu| is most useful for runtime use but is a static database access routine. This routine just finds a menu with the 
given name.
...
\begin{itemize}
\item \index{DCT\_LINK\_CONSTANT}DCT\_LINK\_CONSTANT: Constant value.

\item \index{DCT\_LINK\_PV}DCT\_LINK\_PV: A process variable link.\index{DCT\_LINK\_DEVICE}

\item \index{DCT\_LINK\_FORM}DCT\_LINK\_FORM: A link that can only be processed via the form routines described in the next chapter.
...
\index{dbGetFormPrompt}
\index{dbGetFormValue}
\index{dbPutForm}
...
\index{dbVerifyForm}\verb|dbVerifyForm| can be called to verify user input. It returns \verb|NULL| if no errors are present. If errors are present, it returns 
...
\index{dbGetInfoName}Returns the name of the info item referred to by \verb|pdbentry|, or a NULL pointer if no item is referred to.
...
\index{dbGetInfo}It is common to want to look up the value of a named info item in one call, and \verb|dbGetInfo| is provided for this purpose. 
...
\index{dbFindBrkTable}This routine returns the address of the specified breakpoint table. It is normally used by the runtime breakpoint conversion 
...
The example shows how to iterate through the record types and instances and display field values.
