...
Before any record field is accessed, the record must be lock with a call to either \verb|dbScanLock| or \verb|dbScanLockMany|.
...
It is possible to lock multiple records using \verb|dbScanLockMany|.
First a \verb|dbLocker*| is obtained from an array of record pointers,
then kept and used several times with \verb|dbScanLockMany|.
...
before calling \verb|dbScanLockMany| again.

\verb|dbScanLock| may be called recursively as described in section \ref{sec:recursivelocking}.

The first argument of \verb|dbScanLockMany| is an array of \verb|dbCommon*|
and the second is the number of elements of this array.
This array may contain duplicate elements.
Elements may be NULL.

The third argument of \verb|dbScanLockMany| (\verb|flag|) must be zero as no flags are defined
at present.

\subsection{Recursive Locking}
\label{sec:recursivelocking}

Recursive locking is when a thread attempts to lock a record which it has already locked.
As for example:
...
A record is locked during scanning.
So Device and Record Support code must never call \verb|dbScanLock| nor \verb|dbScanLockMany| from within
a support callback function.

However, asynchronous device support may explicitly call \verb|dbScanLock| when the asynchronous
operation completes from a user thread or \verb|CALLBACK|.

The functions \verb|dbPutField| and \verb|dbGetField| implicitly calls \verb|dbScanLock| or \verb|dbScanLockMany|.
...
\section{Locking Algorithms}
\label{chap:locking algo}

\index{dbScanLock}
\index{dbScanLockMany}
This section describes details of the implementation of \verb|dbScanLock| and \verb|dbScanLockMany|.
Any discussion of links and linking in this section refers only to database link (\verb|DB_LINK|).
Other link types do not effect locking.

A lockset guards one or more records with an \verb|epicMutexId|.
Each lockset maintains a list of its member records.

\paragraph{The relationship between a record and a lockset}
forms the basis of the locking algorithms.
Every record is always a member of some lockset throughout its lifetime.
However, a record may move between locksets.
The relationship between record and lockset is established in the \verb|lockRecord*| private structure
which is the \verb|LSET| field of each record.
Each \verb|lockRecord| structure includes an \verb|epicsSpin*| to maintain its consistency.

\paragraph{Records are associated}
with each other through links with the \verb|DBF_INLINK|, \verb|DBF_OUTLINK|, and \verb|DBF_FWDLINK| field types.
...
Records which are not connected by a database link (directly or indirectly)
are placed in different locksets.
This enable parallel scanning of unrelated processing chains.

When a database link is created between two records in two different locksets,
all the records in these two locksets are merged into one.
The other (empty) lockset is free'd.
...
If this occurs, a new lockset is created and populated with one set of connected records.
This is referred to as a split operation.

\paragraph{Access and modification of the association}
between record and lockset is governed by the following rules:
...
The spinlock must be locked first in order to find the records current lockset.
However, the spinlock must be unlocked before the lockset is locked.
Care must be taken as the association may change if neither is locked.
Further, when two locksets are merged, one will be free'd.

To handle this safely each lockset is given a reference counter.
The lockset will only be free'd when this count falls to zero.
...
There remains the possibility that the association between record and lockset may change
during the moment between unlocking the spinlock and locking the mutex.
This can be detected after the mutex is lock.
When it occurs, the operation must be re-tried.

This assumes that database link modification is a relatively rare operation.

\paragraph{Locking multiple locksets}
is necessary when a database link is created.
The underlying \verb|epicsMutexId| only allows a thread to lock one mutex at a time.
Care must be taken to avoid a deadlock when locking the second, and beyond.

Two common strategies to avoid deadlocks are to use a try-lock operation with ownership tracking,
or to establish a global ordering.
At present the second is used.
...
\paragraph{Merging two locksets}
when a link is created is accomplished by locking both locksets,
then concatenating their record lists into one.
This leaves one empty lockset.

\paragraph{Splitting one lockset into two}
when a link is broken requires finding if the lockset has become partitioned.
When doing this it is helpful to recognize that removing one link between two records (say `A' and `B') will result in
at most two locksets.

To determine if a lockset is partitioned it is sufficient to start with one of the two records (`A'),
then recursively traverse the remaining links to or from record `A'.
If record `B' is encountered during this traversal, then the lockset is not partitioned.
If all records connected to `A' are traversed without finding `B', then the lockset has been partitioned.
All those records connected with `A' become one lockset,
and any remaining records (including `B') become the second.

\paragraph{During IOC startup}
the list of records is iterated (with \verb|dbLockInitRecords|) and lockset are created and populated
for the links defined in loaded .db files.
