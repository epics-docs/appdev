...
\# & A `\#' as the first non-whitespace character on a line marks the beginning of a comment, which continues to the end of the line (however some versions of Base may require a space after the `\#' character to properly recognize it as a comment). If the '#' character is immediately followed by a '-', the commented line will not be echoed into the IOC shell output.\\
