...
\item [promptgroup] The gui group to which the field belongs, for database configuration tools (DCTs).
To support DCTs in sorting the gui groups, the names used in Base start with a two-digit number,
followed by a space-dash-space sequence.
...
\item [gui\_group] The gui group to which the field belongs, for database configuration tools (DCTs).
Gui groups allow DCTs to group fields of a record by functionality, for convenient user access.

The gui group is defined only for fields that can be set by DCTs.

There is no limited fixed set of allowed groups.
Instead, gui group tags are collected when parsing the DBD information, and used to populate a global list.
To support sorting and handling of groups, the names used in Base use the following conventions:

\begin{itemize}
\item Names start with a two-digit number followed by a space-dash-space sequence.
This allows consistent sorting of groups.
\item In many-of-the-same-kind cases (e.g. 21 similar inputs) fields are distributed over multiple groups.
Once-only fields appear in the group that starts with a "major" number (\verb|<n>|0 or \verb|<n>|5).
...
Record types may define their specific gui groups.
However, to improve consistency, records should use the following groups from Base if possible.
(This set also shows an alternative group sharing an existing sorting number.)
...
NOTE: Older versions of Base contained a header file \verb|guigroup.h| defining a fixed set of gui groups strings
and their matching index numbers.
This file has been removed. Instead, the static database access library provides functions to convert between
numerical codes and string tags. See \ref{subsec:Get Field Prompt} for details.
